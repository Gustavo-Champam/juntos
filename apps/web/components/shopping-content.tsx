...
export function ShoppingContent() {
  const [checkedItems, setCheckedItems] = useState<ReadonlySet<string>>(
    () => new Set(shoppingItems.filter((item) => item.checked).map((item) => item.name)),
  );
      ...
      <fieldset className="shopping-list">
        ...
        {shoppingItems.map((item) => (
          <label className="shopping-row" key={item.name}>
            <input type="checkbox" checked={checkedItems.has(item.name)} onChange={() => toggleItem(item.name)} />
            <span className="shopping-check" aria-hidden="true" />
            <span className="shopping-copy"><strong>{item.name}</strong><small>{item.note}</small></span>
